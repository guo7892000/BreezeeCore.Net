...
namespace Breezee.Framework.Mini.DAL.SQLite
{
    ...
    /// 类
    /// </summary>
    public class MiniDalSQLiteIocModuleAutofac : Autofac.Module
    {
        protected override void Load(ContainerBuilder builder)
        {
            DbServerInfo serverInfo = new DbServerInfo(DataBaseType.SQLite, "SQLite_Mini.db", "", "", "", "", "", "");

            builder.RegisterInstance(AutoSQLExecutors.Connect(serverInfo)).As<IDataAccess>().Named<IDataAccess>(MiniGlobalValue.DataAccessConfigKey);
            builder.RegisterType<DMiniLogin>().As<IDMiniLogin>();
            builder.RegisterType<DMiniSystem>().As<IDMiniSystem>();
            builder.RegisterType<DMiniDBInitializer>().As<IDBInitializer>().Named<IDBInitializer>(MiniGlobalValue.DBInitializerConfigKey);
        }
    }
}