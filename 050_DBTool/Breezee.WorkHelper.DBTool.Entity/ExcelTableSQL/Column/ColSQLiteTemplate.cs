    ...
    public class ColSQLiteTemplate
    {
        ...
        public static bool ValidateData(DataTable dtTable, DataTable dtAllCol, GenerateParamEntity paramEntity, out StringBuilder sb)
        {
            ...
            foreach (DataRow drNew in drNewArray)
            {
                ...
                if (dtAllCol.Select(sFilter).Length > 0)
                {
                ...
                if (dtAllCol.Select(EntTable.ExcelTable.Code + "='" + strTableCode + "' and 键='FK' and (" + ExcelCol.FK + " is null or " + ExcelCol.FKName + " is null)").Length > 0)
                {
