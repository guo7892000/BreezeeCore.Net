...
{
	[Serializable]
	public partial class DT_BAS_VALUE : DbEntity
	{

		//自动生成表名
		public static string TName = "BAS_VALUE"; ////本身实例
		public static DbEntity NewEntity(){ return new DT_BAS_VALUE();} ////本身实例
		 public override DbObjectType EntType {  get {return DbObjectType.Table;} }
		 public override string DBTableName {  get {return "BAS_VALUE";} }
		//自动生成字段
		 ...
		 public DbField EF_MODIFY_TYPE = new DbField().DBColName("MODIFY_TYPE").DBColType("VARCHAR").DBTypeSize("VARCHAR(36)").DBScale(-1).Nullable(true).PK(false).DBLen(36).FieldName("").FieldType(EntityFieldType.DbColumn).UpdateControl(false);
		 ...
		 public static string VALUE_CODE = "VALUE_CODE";
		 public DbField EF_VALUE_CODE = new DbField().DBColName("VALUE_CODE").DBColType("VARCHAR").DBTypeSize("VARCHAR(36)").DBScale(-1).Nullable(true).PK(false).DBLen(36).FieldName("").FieldType(EntityFieldType.DbColumn).UpdateControl(false);

		 public static string VALUE_DESC = "VALUE_DESC";
		 public DbField EF_VALUE_DESC = new DbField().DBColName("VALUE_DESC").DBColType("VARCHAR").DBTypeSize("VARCHAR(36)").DBScale(-1).Nullable(true).PK(false).DBLen(36).FieldName("").FieldType(EntityFieldType.DbColumn).UpdateControl(false);

		 public static string VALUE_ID = "VALUE_ID";
		 public DbField EF_VALUE_ID = new DbField().DBColName("VALUE_ID").DBColType("VARCHAR").DBTypeSize("VARCHAR(36)").DBScale(-1).Nullable(true).PK(false).DBLen(36).FieldName("").FieldType(EntityFieldType.DbColumn).UpdateControl(false);

		 public static string VALUE_NAME = "VALUE_NAME";
		 public DbField EF_VALUE_NAME = new DbField().DBColName("VALUE_NAME").DBColType("VARCHAR").DBTypeSize("VARCHAR(36)").DBScale(-1).Nullable(true).PK(false).DBLen(36).FieldName("").FieldType(EntityFieldType.DbColumn).UpdateControl(false);
		 ...
		 public override List<DbField> DbColumnList
		 {
			get
			{
				 ...
				 DbColumn.AddRange(new DbField[] { EF_BELONG_OBJECT_TYPE,EF_CREATE_TIME,EF_CREATOR,EF_CREATOR_ID,EF_EXTEND_VALUE1,EF_EXTEND_VALUE2,EF_EXTEND_VALUE3,EF_EXTEND_VALUE4,EF_EXTEND_VALUE5,EF_IS_ENABLED,EF_IS_SYSTEM,EF_LAST_UPDATED_TIME,EF_MODIFIER,EF_MODIFIER_ID,EF_MODIFY_TYPE,EF_ORG_ID,EF_REMARK,EF_SORT_ID,EF_TFLAG,EF_TYPE_CODE,EF_TYPE_ID,EF_TYPE_NAME,EF_UPDATE_CONTROL_ID,EF_VALUE_CODE,EF_VALUE_DESC,EF_VALUE_ID,EF_VALUE_NAME });
				 return DbColumn;
			}
		 }
		 public override List<string> ColumnStringList
		 {
			get
			{
				 ...
				 DbColumn.AddRange(new string[] { "BELONG_OBJECT_TYPE","CREATE_TIME","CREATOR","CREATOR_ID","EXTEND_VALUE1","EXTEND_VALUE2","EXTEND_VALUE3","EXTEND_VALUE4","EXTEND_VALUE5","IS_ENABLED","IS_SYSTEM","LAST_UPDATED_TIME","MODIFIER","MODIFIER_ID","MODIFY_TYPE","ORG_ID","REMARK","SORT_ID","TFLAG","TYPE_CODE","TYPE_ID","TYPE_NAME","UPDATE_CONTROL_ID","VALUE_CODE","VALUE_DESC","VALUE_ID","VALUE_NAME" });
				 return DbColumn;
			}
		 }
	}
