...
{
	[Serializable]
	public partial class DT_SYS_FORM : DbEntity
	{

		//自动生成表名
		public static string TName = "SYS_FORM"; ////本身实例
		public static DbEntity NewEntity(){ return new DT_SYS_FORM();} ////本身实例
		 public override DbObjectType EntType {  get {return DbObjectType.Table;} }
		 public override string DBTableName {  get {return "SYS_FORM";} }
		//自动生成字段
		 ...
		 public DbField EF_FORM_NAME = new DbField().DBColName("FORM_NAME").DBColType("VARCHAR").DBTypeSize("VARCHAR(36)").DBScale(-1).Nullable(true).PK(false).DBLen(36).FieldName("").FieldType(EntityFieldType.DbColumn).UpdateControl(false);
		 ...
		 public override List<DbField> DbColumnList
		 {
			get
			{
				 ...
				 DbColumn.AddRange(new DbField[] { EF_CREATE_TIME,EF_CREATOR,EF_CREATOR_ID,EF_DLL_ID,EF_FORM_FULL_PATH,EF_FORM_ID,EF_FORM_NAME,EF_IS_ENABLED,EF_IS_SYSTEM,EF_LAST_UPDATED_TIME,EF_MODIFIER,EF_MODIFIER_ID,EF_ORG_ID,EF_REMARK,EF_SORT_ID,EF_TFLAG,EF_UPDATE_CONTROL_ID });
				 return DbColumn;
			}
		 }
		 public override List<string> ColumnStringList
		 {
			get
			{
				 ...
				 DbColumn.AddRange(new string[] { "CREATE_TIME","CREATOR","CREATOR_ID","DLL_ID","FORM_FULL_PATH","FORM_ID","FORM_NAME","IS_ENABLED","IS_SYSTEM","LAST_UPDATED_TIME","MODIFIER","MODIFIER_ID","ORG_ID","REMARK","SORT_ID","TFLAG","UPDATE_CONTROL_ID" });
				 return DbColumn;
			}
		 }
	}
